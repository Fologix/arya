...
if (isset($_POST['submit'])) {
    ...
    if (!empty($prenom) && !empty($email) && !empty($password) && !empty($confirm_password)) {
        if ($password === $confirm_password) {
            if (check_password_strength($password)) {
                // Vérification de l'adresse e-mail dans la base de données
                $sql_check_email = "SELECT * FROM vendeur WHERE mail = :mail";
                $stmt_check_email = $pdo->prepare($sql_check_email);
                $stmt_check_email->execute(['mail' => $email]);
                $existing_admin = $stmt_check_email->fetch();

                if (!$existing_admin) {
                    $password = password_hash($password, PASSWORD_DEFAULT);
                    $sql = "INSERT INTO vendeur (prenom_vendeur, mail, password) VALUES (:prenom, :mail, :password)";
                    ...
                    $message = "Ajout d'administrateur réussi !";
                } else {
